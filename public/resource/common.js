...
function slideMenuOnOff_init(){
    $('header > nav > .cate-menu').click(slideMenuOn);
    $('#slide-menu-wrap div.close-btn').click(slideMenuOff);
    $('#slide-menu-wrap div.slide-menu-bg').click(slideMenuOff);
   
    function slideMenuOn(){
       $('#slide-menu-wrap').addClass('on');
       $('html, body').addClass('on');
    }
   
    function slideMenuOff(){
       $('#slide-menu-wrap').removeClass('on');
       $('html, body').removeClass('on');
      
    }
}

function scrollAnimation_init(){

    function scrollAnimation01(){
        var controller = new ScrollMagic.Controller();
    
        var tween1 = TweenMax.to('.article-box-wrap', 0.7, {
            opacity: 1,
    ...
    $(window).scroll(function(){
        if ($(document).scrollTop() > $header.top + ($('#header').outerHeight() / 2)  ){
            $('#header').addClass('fixed');
            $('#contents').addClass('header-fixed');

        }
        else{
            $('#header').removeClass('fixed');
            $('#contents').removeClass('header-fixed');
        }
